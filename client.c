...
void selectOption(int sd);
void showOptions(int sd);
...
int main(){
	struct sockaddr_in ser;       // The <netinet/in.h> header defines the sockaddr_in structure
	int sd,msglen;
	char buff[50];
	char result;
	...
	ser.sin_addr.s_addr=inet_addr("127.0.0.1");  //same machine  // If inet_addr is successful, it returns the address in network byte order.
	ser.sin_port=htons(5555);   // the htons() function converts values between host and network byte orders. These routines convert 16 and 32 bit quantities between network byte order and host byte order. (Network byte order is big endian, or most significant byte first.)

	connect(sd,(struct sockaddr *)&ser,sizeof(ser));   // int connect(int socket, const struct sockaddr *address, socklen_t address_len);
	selectOption(sd);
	showOptions(sd);
	close(sd);
	return 0;
